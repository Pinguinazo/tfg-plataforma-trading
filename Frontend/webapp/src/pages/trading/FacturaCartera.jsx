...
export const printFiatInvoice = (transactionsArray, globalUser, rate, sym) => {
  ...
  const tableColumn = ["Fecha Operación", "Tipo", "Concepto", "Importe"];
  const tableRows = [];
  let grandTotal = 0;
  ...
  transactionsArray.forEach(tx => {
    ...
    const tipoText = isDeposit ? 'DEPÓSITO' : 'RETIRO';
    const concept = isDeposit ? 'Ingreso a cuenta' : 'Retiro de fondos';
    
    const amountLocal = parseFloat(tx.amount) * rate;
    grandTotal += amountLocal; 
    ...
    tableRows.push([
      ...
      tipoText,
      `${concept} (Simulación)`,
      `${amountLocal.toFixed(2)}${sym}`
    ]);
  });

  tableRows.push([
    { content: 'Volumen Total Procesado:', colSpan: 3, styles: { halign: 'right', fontStyle: 'bold' } },
  ...
  doc.save(`Justificante_transacciones_${Date.now()}.pdf`);
};